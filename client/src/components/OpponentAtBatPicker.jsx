...
import { RESULT_GROUPS, RESULT_CODES } from '../utils/constants.js';
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, opacity: isHbp ? 0.45 : 1 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
            <IconButton size="small" onClick={() => onFoulsChange(Math.max(0, fouls - 1))} disabled={isHbp || fouls <= 0} sx={{ bgcolor: 'grey.200', width: 28, height: 28 }}>
              <RemoveIcon sx={{ fontSize: 14 }} />
            </IconButton>
            <Typography sx={{ fontFamily: '"Roboto Mono", monospace', fontWeight: 700, minWidth: 20, textAlign: 'center' }}>
              {fouls}
            </Typography>
            <IconButton size="small" onClick={() => onFoulsChange(fouls + 1)} disabled={isHbp} sx={{ bgcolor: 'grey.200', width: 28, height: 28 }}>
...
export default function OpponentAtBatPicker({
  open, onClose, onConfirm, batterName, inning, maxInning = 9, pitcherOptions = [],
}) {
  const [selectedResult, setSelectedResult] = useState(null);
  const [selectedInning, setSelectedInning] = useState(inning || 1);
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 3, mx: 1, maxHeight: '92vh' } }}>
      <DialogTitle sx={{ pb: 1, pt: 2, px: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <Typography variant="h6" sx={{ fontWeight: 700, lineHeight: 1.2 }}>상대팀 타석 기록</Typography>
          {batterName && (
            <Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>
              타자: <strong>{batterName}</strong>
            </Typography>
          )}
        </Box>
      ...
      <DialogContent sx={{ px: 2, pt: 0, pb: 1 }}>
        ...
        <FormControl fullWidth size="small" sx={{ mb: 2 }}>
          <InputLabel>담당 투수</InputLabel>
          <Select value={pitcherId} label="담당 투수" onChange={(e) => setPitcherId(e.target.value)}>
        ...
        {/* 볼 카운트 섹션 */}
        <BallCountSection
          balls={balls} strikes={strikes} fouls={fouls} result={selectedResult}
          onBallsChange={setBalls} onStrikesChange={setStrikes} onFoulsChange={setFouls}
        />
