...
function ResultButton({ code, selected, onSelect }) {
  ...
  return (
    ...
    >
      <Typography
        variant="body1"
        sx={{ fontWeight: 800, fontSize: '1rem', fontFamily: '"Roboto Mono", monospace', lineHeight: 1 }}
      >
        {code}
      </Typography>
      <Typography
        variant="caption"
        sx={{ fontSize: '0.65rem', fontWeight: 600, lineHeight: 1, opacity: 0.9 }}
      >
        {info?.label}
      </Typography>
    </Button>
...
function Counter({ label, value, onChange, min = 0, max = 9 }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      <Typography
        sx={{ fontFamily: '"Roboto Mono", monospace', fontWeight: 700, fontSize: '1.25rem', minWidth: 28, textAlign: 'center' }}
      >
        {value}
      </Typography>
      <IconButton
 ...
 *   open           - boolean
 *   onClose        - () => void
 *   onConfirm      - ({ result, inning, rbi, run, note, pitcherId }) => void
 *   batterName     - string  (opponent batter name)
 *   inning         - number  (currently selected inning)
 *   maxInning      - number
 *   pitcherOptions - array of { id, name, number } (our team pitchers)
 */
export default function OpponentAtBatPicker({
  ...
  const handleConfirm = () => {
    ...
    onConfirm({ result: selectedResult, inning: selectedInning, rbi, run, note, pitcherId });
    reset();
  };
